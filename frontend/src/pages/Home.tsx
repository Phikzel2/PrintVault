...
export function Home() {
  ...
  return (
    ...
    >
      ...
      {selectedIds.size > 0 && (
        <div
          className="fixed bottom-0 left-0 right-0 md:bottom-6 md:left-1/2 md:right-auto md:-translate-x-1/2 md:w-auto z-50 bg-white dark:bg-gray-900 border-t md:border border-gray-200 dark:border-gray-700 md:rounded-2xl shadow-2xl px-4 md:px-5 pt-3 md:py-3"
          style={{ paddingBottom: "max(12px, env(safe-area-inset-bottom))" }}
        >
          {confirmBatchDelete ? (
            /* Confirm delete — same layout on mobile and desktop */
            <div className="flex items-center justify-between gap-3">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                Delete {selectedIds.size} model{selectedIds.size !== 1 ? "s" : ""}?
              </span>
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => setConfirmBatchDelete(false)} className="text-sm px-3 py-1.5 btn-secondary">
                  Cancel
                </button>
            ...
            <>
              ...
              <div className="md:hidden space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={clearSelection} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors w-8 h-8 flex items-center justify-center rounded-lg text-lg">✕</button>
                  </div>
                </div>
                {/* Row 2: tag */}
                <form onSubmit={handleBatchTag} className="flex items-center gap-2">
                  <input
                    type="text"
                    value={batchTag}
                    onChange={e => setBatchTag(e.target.value)}
                    placeholder="Add tag…"
                    className="text-sm border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1.5 bg-transparent focus:outline-none focus:ring-1 focus:ring-brand-500 flex-1"
                  />
                  <button type="submit" disabled={batchBusy || !batchTag.trim()}
                    className="btn-secondary text-xs py-1.5 px-3 disabled:opacity-50 shrink-0">
                    Tag
                  </button>
                </form>
              ...
              <div className="hidden md:flex items-center gap-3">
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300 shrink-0">{selectedIds.size} selected</span>
                <button
                  onClick={() => selectedIds.size === models.length ? setSelectedIds(new Set()) : setSelectedIds(new Set(models.map(m => m.id)))}
                ...
                <div className="w-px h-5 bg-gray-200 dark:bg-gray-700 shrink-0" />
                <button onClick={clearSelection} className="text-sm text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors shrink-0">✕</button>
              </div>
            </>
          )}
        </div>
